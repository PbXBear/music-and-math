...
// 计算和弦得分
void cal_chord_maj(const int beat[HALF], int idx, const chord_t *prev, int nowscore)
{
    if (idx >= 9) // 搜到底
    {
...
void cal_chord_min(const int beat[HALF], int idx, const chord_t *prev, int nowscore)
{
    if (idx >= 9) // 搜到底
    {
        if (nowscore > nowbest) // 更新nowbest
